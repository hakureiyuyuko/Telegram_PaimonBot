...
async def mihoyo_msg(client: Client, message: Message):
    ...
    if '添加' in text:
        try:
            ...
            if not mes:
                return await message.reply_text("获取 Cookie 请登录：[这个链接](https://account.mihoyo.com/#/login) 后按F12运行<code>var cookie=document.cookie;var ask=confirm('Cookie:'+cookie+'\n\nDo you want to copy the cookie to the clipboard?');if(ask==true){copy(cookie);msg=cookie}else{msg='Cancel'}</code>", quote=True)
